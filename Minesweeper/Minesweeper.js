var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }();

function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

function _possibleConstructorReturn(self, call) { if (!self) { throw new ReferenceError("this hasn't been initialised - super() hasn't been called"); } return call && (typeof call === "object" || typeof call === "function") ? call : self; }

function _inherits(subClass, superClass) { if (typeof superClass !== "function" && superClass !== null) { throw new TypeError("Super expression must either be null or a function, not " + typeof superClass); } subClass.prototype = Object.create(superClass && superClass.prototype, { constructor: { value: subClass, enumerable: false, writable: true, configurable: true } }); if (superClass) Object.setPrototypeOf ? Object.setPrototypeOf(subClass, superClass) : subClass.__proto__ = superClass; }

import React from 'react';
import Board from './Board';
import Menu from './Menu';
import './Minesweeper.css';

var Minesweeper = function (_React$Component) {
    _inherits(Minesweeper, _React$Component);

    function Minesweeper(props) {
        _classCallCheck(this, Minesweeper);

        var _this = _possibleConstructorReturn(this, (Minesweeper.__proto__ || Object.getPrototypeOf(Minesweeper)).call(this, props));

        var newRows = 10;
        var newCols = 10;
        var numMines = Math.floor(0.15 * newRows * newCols);
        var newBoard = _this.generateBoard(newRows, newCols);
        _this.state = {
            difficulty: 'easy',
            numMines: numMines,
            board: newBoard,
            mines: new Array(newRows).fill(new Array(newCols)),
            rows: newRows,
            cols: newCols,
            gameState: 'menu'
        };
        return _this;
    }

    _createClass(Minesweeper, [{
        key: 'setGameState',
        value: function setGameState(state) {
            if (state === 'play') {
                this.cleanBoard();
            }
            this.setState({
                gameState: state
            });
        }
    }, {
        key: 'setDifficulty',
        value: function setDifficulty(newDifficulty) {
            var newRows = void 0;
            var newCols = void 0;
            switch (newDifficulty) {
                case 'easy':
                    newRows = 10;
                    newCols = 10;
                    break;
                case 'medium':
                    newRows = 15;
                    newCols = 15;
                    break;
                case 'hard':
                    newRows = 20;
                    newCols = 20;
                    break;
            }
            var newNumMines = Math.floor(0.15 * newRows * newCols);
            var newBoard = this.generateBoard(newRows, newCols);
            this.setState({
                difficulty: newDifficulty,
                numMines: newNumMines,
                board: newBoard,
                mines: new Array(newRows).fill(new Array(newCols)),
                rows: newRows,
                cols: newCols
            });
        }
    }, {
        key: 'cleanBoard',
        value: function cleanBoard() {
            var newNumMines = Math.floor(0.15 * this.state.rows * this.state.cols);
            var newBoard = this.generateBoard(this.state.rows, this.state.cols);
            this.setState({
                numMines: newNumMines,
                board: newBoard,
                firstClick: true
            });
        }

        // generates board of ints 
        // covered: 0
        // uncovered: 1
        // marked: 2
        // exploded: 3

    }, {
        key: 'generateBoard',
        value: function generateBoard(rows, cols) {
            var board = new Array(rows);
            for (var i = 0; i < board.length; i++) {
                board[i] = new Array(cols).fill(0);
            }
            return board;
        }

        // Exclusively for visualizing and testing mine generation

    }, {
        key: 'generateUncoveredBoard',
        value: function generateUncoveredBoard(mines) {
            var board = new Array(this.props.rows);
            for (var i = 0; i < board.length; i++) {
                board[i] = new Array(this.props.cols).fill(1);
                for (var j = 0; j < board[i].length; j++) {
                    if (mines[i][j] === -1) {
                        board[i][j] = 2;
                    }
                }
            }
            return board;
        }

        // Generates mines and board numbers

    }, {
        key: 'generateMinesOnClick',
        value: function generateMinesOnClick(row, col) {
            console.log('generating mines');
            // Initialize empty 2D array
            var mines = new Array(this.state.rows);
            for (var i = 0; i < mines.length; i++) {
                mines[i] = new Array(this.state.cols).fill(0);
            }

            // Add mines until numMines has been reached
            var mineCount = 0;
            console.log(this.state.numMines);
            while (mineCount < this.state.numMines) {

                // select random square
                var r = Math.floor(Math.random() * this.state.rows);
                var c = Math.floor(Math.random() * this.state.cols);

                // designate random square as a mine if it is not close to first click and is not in saturated area
                if (mines[r][c] !== -1 && !this.isAdjacent(row, col, r, c) && !this.areaIsSaturated(r, c, mines)) {
                    mines[r][c] = -1;
                    mineCount++;

                    // update numbers around new mine
                    for (var _i = r - 1; _i < r + 2; _i++) {
                        for (var j = c - 1; j < c + 2; j++) {
                            if (this.coordinatesOnBoard(_i, j) && mines[_i][j] !== -1) {
                                mines[_i][j]++;
                            }
                        }
                    }
                }
            }
            this.setState({
                firstClick: false
            });
            console.log(mines);
            return mines;
        }

        // for checking if two squares are adjacent to each other

    }, {
        key: 'isAdjacent',
        value: function isAdjacent(r1, c1, r2, c2) {
            var length1 = Math.pow(Math.abs(r2 - r1), 2);
            var length2 = Math.pow(Math.abs(c2 - c1), 2);
            return Math.sqrt(length1 + length2) < 3;
        }

        // For checking if the area around (r, c) is saturated with mines 
        // and distributing mines in an optimal way

    }, {
        key: 'areaIsSaturated',
        value: function areaIsSaturated(r, c, mines) {
            var adjWeightSum = 0;
            var adjMineSum = 0;
            for (var i = r - 1; i < r + 2; i++) {
                for (var j = c - 1; j < c + 2; j++) {
                    if (this.coordinatesOnBoard(i, j)) {
                        adjWeightSum += mines[i][j];
                        if (mines[i][j] === -1) {
                            adjMineSum++;
                        }
                    }
                }
            }
            return adjWeightSum > 9 || adjMineSum > 2;
        }

        // for checking if (r, c) are on the current board.

    }, {
        key: 'coordinatesOnBoard',
        value: function coordinatesOnBoard(r, c) {
            return r >= 0 && r < this.state.rows && c >= 0 && c < this.state.cols;
        }

        // for testing that valid mine board has been generated

    }, {
        key: 'verifyNums',
        value: function verifyNums(mines) {
            for (var r = 0; r < this.props.rows; r++) {
                for (var c = 0; c < this.props.cols; c++) {
                    if (mines[r][c] !== -1) {
                        var adjMines = 0;
                        for (var i = r - 1; i < r + 2; i++) {
                            for (var j = c - 1; j < c + 2; j++) {
                                if (this.coordinatesOnBoard(i, j) && mines[i][j] === -1) {
                                    adjMines++;
                                }
                            }
                        }
                        if (adjMines !== mines[r][c]) {
                            return false;
                        }
                    }
                }
            }
            return true;
        }
    }, {
        key: 'explodeMines',
        value: function explodeMines(board, mines) {
            for (var r = 0; r < this.state.rows; r++) {
                for (var c = 0; c < this.state.cols; c++) {
                    if (mines[r][c] === -1) {
                        board[r][c] = 3;
                    } else {
                        board[r][c] = 1;
                    }
                }
            }
        }

        // uncovered adjacent zero squares for zeros

    }, {
        key: 'uncoverAdjacentSquares',
        value: function uncoverAdjacentSquares(r, c, newBoard, mines) {
            if (mines[r][c] === 0) {
                for (var i = r - 1; i < r + 2; i++) {
                    for (var j = c - 1; j < c + 2; j++) {
                        if (this.coordinatesOnBoard(i, j) && newBoard[i][j] === 0) {
                            newBoard[i][j] = 1;
                            if (!(i === r && j === c) && mines[i][j] === 0) {
                                this.uncoverAdjacentSquares(i, j, newBoard, mines);
                            }
                        }
                    }
                }
            }
        }

        // returns boolean of if winning game state has been reached

    }, {
        key: 'gameWon',
        value: function gameWon(board) {
            if (this.props.gameState === 'lost') {
                return false;
            }
            for (var r = 0; r < this.state.rows; r++) {
                for (var c = 0; c < this.state.cols; c++) {
                    if (board[r][c] === 0) {
                        return false;
                    }
                }
            }
            return true;
        }

        // Makes the appropriate changes to the board when a square (r, c) is clicked

    }, {
        key: 'handleClick',
        value: function handleClick(r, c) {
            // console.log(this.);
            if (this.state.gameState === 'play') {
                var newBoard = this.state.board;
                var newMines = this.state.mines;
                var newGameState = this.state.gameState;

                // Generate mines if this is the start of the game
                if (this.state.firstClick) {
                    newMines = this.generateMinesOnClick(r, c);
                }

                // if a covered square was clicked, take the appropriate action
                if (this.state.board[r][c] === 0) {

                    // for uncovering the square and adjacent squares if necessary
                    if (newMines[r][c] !== -1) {
                        newBoard[r][c] = 1;
                        this.uncoverAdjacentSquares(r, c, newBoard, newMines);

                        // check for winning game state
                        if (this.gameWon(newBoard)) {
                            newGameState = 'won';
                        }
                    } else if (newMines[r][c] === -1) {

                        // for ending the game if mine was clicked
                        newGameState = 'lost';
                        this.explodeMines(newBoard, newMines);
                    }
                }

                // for testing
                // newBoard = this.generateUncoveredBoard(newMines); 

                this.setState({
                    board: newBoard,
                    mines: newMines,
                    gameState: newGameState
                });
            }
        }

        // takes the appropriate action when a square is right clicked

    }, {
        key: 'handleRightClick',
        value: function handleRightClick(r, c) {
            if (this.state.gameState === 'play') {
                var newBoard = this.state.board;
                var newGameState = this.state.gameState;

                // mark or unmark the square depending on it's current state
                if (this.state.board[r][c] === 0) {
                    newBoard[r][c] = 2;
                } else if (this.state.board[r][c] === 2) {
                    newBoard[r][c] = 0;
                }
                if (this.gameWon(newBoard)) {
                    newGameState = 'won';
                }
                this.setState({
                    board: newBoard,
                    gameState: newGameState
                });
            }
        }
    }, {
        key: 'render',
        value: function render() {
            var _this2 = this;

            return React.createElement(
                'div',
                { className: 'game' },
                React.createElement(
                    'div',
                    { className: 'title' },
                    'Minesweeper'
                ),
                React.createElement(Menu, {
                    gameState: this.state.gameState,
                    difficulty: this.state.difficulty,
                    setGameState: function setGameState(state) {
                        return _this2.setGameState(state);
                    },
                    setDifficulty: function setDifficulty(difficulty) {
                        return _this2.setDifficulty(difficulty);
                    }
                }),
                React.createElement(Board, {
                    mines: this.state.mines,
                    board: this.state.board,
                    numMines: this.state.numMines,
                    rows: this.state.rows,
                    cols: this.state.cols,
                    gameState: this.state.gameState,
                    handleClick: function handleClick(r, c) {
                        return _this2.handleClick(r, c);
                    },
                    handleRightClick: function handleRightClick(r, c) {
                        return _this2.handleRightClick(r, c);
                    }
                })
            );
        }
    }]);

    return Minesweeper;
}(React.Component);

export default Minesweeper;